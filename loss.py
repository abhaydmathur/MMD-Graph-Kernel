...
def sc_loss(s, pos_mask, alpha):
    ...
    log_denominator = (
        pos_mask.sum(1) * torch.log(torch.sum(s_denominator, dim=-1))
    ).sum()
...
def get_MDD(source, target, bandwidth: list):
    ...
    for bw in bandwidth:
        ...
        M[:batch_size, :batch_size] = torch.ones(batch_size, batch_size).to(device) / (
            batch_size * batch_size
        )
        M[batch_size:, batch_size:] = torch.ones(batch_size_2, batch_size_2).to(
            device
